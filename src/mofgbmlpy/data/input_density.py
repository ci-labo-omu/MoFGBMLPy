...
from mofgbmlpy.data.dataset_density import DatasetWithDensity
...
class Input_density():
    ...
    def input_data_set(file_name, is_multi_label):
        ...
        if is_multi_label:
            ...
            return Input_density.input_data_set_basic(file_name)
    ...
    def input_data_set_basic(file_name):
        ...
        with open(file_name, newline='') as csvfile:
            ...
            for row in reader:
                ...
                pattern_id += 1
                densities.append(int(row[num_dim + 1]))
        densities = np.array(densities, dtype=int)
        patterns = np.array(patterns, dtype=object)

        dataset = DatasetWithDensity(size, num_dim, num_classes, patterns, densities)
        return dataset
