...
cdef class LearningBasicDensity(AbstractLearningWithDensity):
    ...
    cdef double[:] calc_confidence(self, Antecedent antecedent, DatasetWithDensity dataset=None):
        ...
        cdef int[:] densities = dataset.get_densities()
        cdef int i
        cdef Pattern p
        ...
        for c in range(num_classes):
            ...
            for i in range(dataset.get_size()):
                ...
                if pattern.get_target_class().get_class_label_value() == c:
                    part_sum += compatible_grades[i] * 1/
    ...
    cpdef double[:] calc_confidence_py(self, Antecedent antecedent, DatasetWithDensity dataset=None):
    ...
    def __eq__(self, other):
        ...
        """
        if not isinstance(other, LearningBasicDensity):
            return False
