...
from mofgbmlpy.fuzzy.rule.consequent.learning.learning_basic_density import LearningBasicDensity
...
class AbstractMoFGBMLDensityMain(ABC):
    ...
    def main(self, args, train=None, test=None, trains=None, change=False):
        ...
        if self._mofgbml_args.get("GEN_PLOT"):
            ...
            AbstractMoFGBMLDensityMain.plot_line_interpretability_error_rate_tradeoff(res.opt.get("X")[:, 0],
    ...
    def plot_line_interpretability_error_rate_tradeoff(solutions, file_path=None, title=None, xlim=None, grid=True, x_key="total_rule_length"):
        ...
        AbstractMoFGBMLDensityMain.plot_line_interpretability_error_rate_tradeoff_from_coords(err_train, err_test, x_label=x_label, y_label="Error rate", file_path=file_path, title=title, xlim=xlim, grid=grid)
