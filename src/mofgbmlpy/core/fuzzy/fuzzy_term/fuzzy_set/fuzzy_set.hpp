...
class FuzzySet {
private:
    std::shared_ptr<AbstractMF> function;
    std::string term;
    int id;
    DivisionType division_type;

public:
    FuzzySet(std::shared_ptr<AbstractMF> function, int id, DivisionType division_type, std::string term = "");
    FuzzySet(const FuzzySet& other);
    operator std::string() const;
    std::string to_string() const;

    std::shared_ptr<AbstractMF> get_function() const;
    void set_function(std::shared_ptr<AbstractMF> function);
