...
FuzzyVariable::FuzzyVariable(const std::vector<std::shared_ptr<FuzzySet>>& fuzzy_sets, const std::string& name, const std::vector<float>& domain)
...
FuzzyVariable::FuzzyVariable(const FuzzyVariable& other) {
    ...
    fuzzy_sets = std::vector<std::shared_ptr<FuzzySet>>(other.fuzzy_sets.size());
    for (size_t i = 0; i < other.fuzzy_sets.size(); ++i) {
        fuzzy_sets[i] = std::shared_ptr<FuzzySet>(other.fuzzy_sets[i]->clone());
    }
}
...
std::shared_ptr<FuzzySet> FuzzyVariable::get_fuzzy_set(int fuzzy_set_index) const {
...
FuzzyVariable::operator std::string() const {
    ...
    for (const std::shared_ptr<FuzzySet>& fs : fuzzy_sets) {
