...
class FuzzyVariable {
private:
    std::vector<FuzzySet*> fuzzy_sets;
    std::string name;
    std::vector<float> domain;
public:
    FuzzyVariable(const std::vector<FuzzySet*>& fuzzy_sets, const std::string& name = "unamed_var", const std::vector<float>& domain = {0.0f, 1.0f});
    FuzzyVariable(const FuzzyVariable& other);
    std::string get_name() const;
    float get_membership_value(int fuzzy_set_index, float x) const;
    int get_length() const;
    FuzzySet* get_fuzzy_set(int fuzzy_set_index) const;
    float get_support(int fuzzy_set_index) const;
    std::vector<FuzzySet*> get_fuzzy_sets() const;
