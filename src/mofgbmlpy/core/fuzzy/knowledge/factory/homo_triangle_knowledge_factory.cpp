    ...
    : num_divisions(num_divisions), var_names(var_names), fuzzy_set_names(fuzzy_set_names)
{
    ...
    int num_dims = num_divisions.size();

    if (var_names.size() != num_dims || fuzzy_set_names.size() != num_dims) {
    ...
    for (int dim_i = 0; dim_i < num_dims; dim_i++) {
        int num_divisions_dim_i = num_divisions[dim_i].size();
        if (fuzzy_set_names[dim_i].size() != num_divisions_dim_i) {
            throw std::invalid_argument("fuzzy_set_names second dimension is invalid");
        }
        for (int j = 0; j < num_divisions_dim_i; ++j) {
            if (fuzzy_set_names[dim_i][j].size() != num_divisions[dim_i][j]) {
...
std::vector<std::vector<float>> HomoTriangleKnowledgeFactory::make_triangle_knowledge_params(int num_fuzzy_sets) {
    ...
    for (int i = 0; i < num_fuzzy_sets; i++) {
        ...
        } else if (i>0 && i < partition.size()-2) { // if the index is valid
...
std::shared_ptr<Knowledge> HomoTriangleKnowledgeFactory::create() {
    auto knowledge = std::make_shared<Knowledge>();

    int set_id = 0;
    std::vector<std::shared_ptr<FuzzyVariable>> fuzzy_vars = std::vector<std::shared_ptr<FuzzyVariable>>(num_divisions.size());

    for (int dim_i = 0; dim_i < num_divisions.size(); dim_i++) {
        std::vector<std::shared_ptr<FuzzySet>> current_set;
        current_set.push_back(std::make_shared<DontCareFuzzySet>(set_id));
        set_id++;

        for (int j = 0; j < num_divisions[dim_i].size(); j++) {
            ...
            for (int div_i = 0; div_i < partition_size; div_i++) {
                ...
                auto fuzzy_set = std::make_shared<TriangularFuzzySet>(left, center, right, set_id, fuzzy_set_names[dim_i][j][div_i]);
                set_id++;
                current_set.push_back(fuzzy_set);
            }
        }
        fuzzy_vars[dim_i] = std::make_shared<FuzzyVariable>(current_set, var_names[dim_i]);
    }
