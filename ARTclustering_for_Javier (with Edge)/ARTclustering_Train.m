...
%
function net = ARTclustering_Train(DATA, net)
...
for sampleNum = 1:size(DATA,1)
    ...
    if size(weight,1) < 1 % In the case of the number of nodes in the entire space is small.
        ...
        if minCIM < Lcim_s1 % Case 1 i.e., V < CIM_k1
            ...
            if minCIM >= Lcim_s2 % Case 3 i.e., V >= CIM_k2
                ...
                for k = s1Neighbors
                    weight(k,:) = weight(k,:) + ( 1/(100*CountNode(k) )) * (input - weight(k,:));
                end
    ...
    % Topology Adjustment
    % If activate the following function, CAE shows a noise reduction ability.
