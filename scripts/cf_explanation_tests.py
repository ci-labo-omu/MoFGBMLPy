...
def get_config(data_name, min_num_rules=None, num_evals=10000):
    args = [
        "--data-name",
        f"{data_name}",
        "--algorithm-id",
...
if __name__ == "__main__":
    ...
    for data_name in ["bupa", "iris", "pima"]:
        ...
        if os.path.exists(result_path):
            print(f"Path {result_path} already exists (skipped).")
            continue
        num_classes, train_dataset, test_dataset, non_dominated_solutions = get_config(data_name, min_num_rules=2)
        main_benchmark(num_classes, non_dominated_solutions, out_path=result_path, test_dataset=test_dataset, data_name=data_name, test_name="classic_min_num_rules_2")
        # except Exception as e:
            # print(f"Error processing {data_name}: {e}")
    ...
    for data_name in ["bupa", "iris", "pima"]:
        ...
        for param_name in ["n_gen", "pop_size"]:
            # try:
            result_path = f"..\\cf_results\\cf_metaheuristics\\param_search\\{param_name}\\{data_name}"
            param_search(num_classes, non_dominated_solutions, result_path, param_name, test_dataset=test_dataset, data_name=data_name, num_experiments=10, min_val=10, max_val=100, is_int=True)
            # except Exception as e:
            #     print(f"Error processing {data_name} for param {param_name}: {e}")
