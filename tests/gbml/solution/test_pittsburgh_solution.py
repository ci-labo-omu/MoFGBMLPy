...
class TestGetErrorRateBasic:
    def test_get_error_rate_all_wins(self):
        dataset = Dataset(
            ...
            np.array(
                [
                    Pattern(0, np.array([0.5, 0.0, 1.0, 0.0], np.float64), ClassLabelBasic(0)),
                    Pattern(1, np.array([0.0, 0.5, 0.0, 1.0], np.float64), ClassLabelBasic(1)),
                    Pattern(2, np.array([1.0, 0.5, 0.0, 0.0], np.float64), ClassLabelBasic(2)),
                    Pattern(3, np.array([0.5, 1.0, 0.0, 0.0], np.float64), ClassLabelBasic(2)),
                ]
            ),
        )
        ...
        sol1 = create_michigan_sol(
            dataset, antecedent_indices=np.copy(antecedent_indices), consequent=copy.deepcopy(consequent)
        )
        ...
        dataset = Dataset(
            ...
            np.array(
                [
                    Pattern(0, np.array([0.5, 0.0, 1.0, 0.0], np.float64), ClassLabelBasic(0)),
                    Pattern(1, np.array([0.0, 0.5, 0.0, 1.0], np.float64), ClassLabelBasic(1)),
                    Pattern(2, np.array([1.0, 0.0, 0.5, 0.0], np.float64), ClassLabelBasic(2)),
                    Pattern(3, np.array([0.0, 1.0, 0.0, 0.5], np.float64), ClassLabelBasic(2)),
                ]
            ),
        )
    ...
    def test_get_error_rate_all_wins_but_one_classification_error(self):
        dataset = Dataset(
            ...
            np.array(
                [
                    ...
                    Pattern(1, np.array([0.0, 0.5, 0.0, 1.0], np.float64), ClassLabelBasic(1)),
                    Pattern(2, np.array([1.0, 0.5, 0.0, 0.0], np.float64), ClassLabelBasic(2)),
                    Pattern(3, np.array([0.5, 1.0, 0.0, 0.0], np.float64), ClassLabelBasic(0)),
                ]
            ),
        )
        ...
        assert (
            error_rate == 1 / 4
            and sol1.get_fitness() == 1
            and sol1.get_num_wins() == 1
            and sol2.get_fitness() == 1
            and sol2.get_num_wins() == 1
            and sol3.get_fitness() == 1
            and sol3.get_num_wins() == 2
        )
...
class TestGetErrorRateMulti:
    def test_get_error_rate_all_wins(self):
        dataset = Dataset(
            ...
            np.array(
                [
                    ...
                    Pattern(3, np.array([0.5, 1.0, 0.0, 0.0], np.float64), ClassLabelMulti(np.array([1, 0, 1]))),
                ]
            ),
        )
        ...
        rule_weight = RuleWeightMulti(np.array([0.7, 0.7, 0.7], np.float64))
        ...
        rule_weight = RuleWeightMulti(np.array([0.5, 0.5, 0.5], np.float64))
        consequent = ConsequentMulti(class_label, rule_weight)
        sol3 = create_michigan_sol(
            dataset,
            antecedent_indices=np.copy(antecedent_indices),
        ...
        dataset = Dataset(
            ...
            np.array(
                [
                    Pattern(0, np.array([0.5, 0.0, 1.0, 0.0], np.float64), ClassLabelMulti(np.array([1, 0, 0]))),
                    Pattern(1, np.array([0.0, 0.5, 0.0, 1.0], np.float64), ClassLabelMulti(np.array([0, 1, 0]))),
                    Pattern(2, np.array([1.0, 0, 0.5, 0.0], np.float64), ClassLabelMulti(np.array([1, 0, 1]))),
                    Pattern(3, np.array([0.0, 1.0, 0.0, 0.5], np.float64), ClassLabelMulti(np.array([1, 0, 1]))),
                ]
            ),
        )
...
def test_deep_copy():
    ...
    michigan_solution_builder = MichiganSolutionBuilder(
        random_gen, 1, 0, RuleBuilderBasic(antecedent_factory, consequent_factory, knowledge)
    )
    ...
    for i in range(obj.get_num_vars()):
        ...
        assert id(v1.get_antecedent().get_antecedent_indices().base) != id(
            v2.get_antecedent().get_antecedent_indices().base
        )
...
def test_to_xml_run():
    ...
    knowledge = HomoTriangleKnowledgeFactory_2_3_4_5(training_data_set.get_num_dim()).create()
    rule_builder = RuleBuilderBasic(
        AllCombinationAntecedentFactory(knowledge, random_gen), LearningBasic(training_data_set), knowledge
    )
