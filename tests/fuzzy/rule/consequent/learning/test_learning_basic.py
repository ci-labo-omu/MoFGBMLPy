    ...
    dataset = Dataset(
        ...
        np.array(
            [
                Pattern(0, np.array([0.5, 0.0, 1.0, 0.0], np.float64), ClassLabelBasic(0)),
                Pattern(1, np.array([0.0, 0.5, 0.0, 1.0], np.float64), ClassLabelBasic(1)),
                Pattern(2, np.array([0.0, 0.5, 0.0, 0.0], np.float64), ClassLabelBasic(2)),
                Pattern(3, np.array([0.5, 1.0, 0.0, 0.0], np.float64), ClassLabelBasic(2)),
            ]
        ),
    )
...
def test_calc_class_label_same_highest():
    confidence = np.array([0.3, 0.2, 0.3, 0.2], np.float64)
    learner = LearningBasic(train)
...
def test_calc_class_label_different_highest():
    confidence = np.array([0.3, 0.0, 0.5, 0.2], np.float64)
    learner = LearningBasic(train)
    label = learner.calc_class_label(confidence)
...
def test_calc_rule_weight_none_class_label():
    confidence = np.array([0.4, 0.0, 0.5, 0.2], np.float64)
    reject_threshold = 0
    learner = LearningBasic(train)
...
def test_calc_rule_weight_incompatible_class_label_and_confidence():
    ...
    confidence = np.array([0.4, 0.1, 0.3, 0.2], np.float64)
    reject_threshold = 0
    learner = LearningBasic(train)
...
def test_calc_rule_weight_equal_0_5():
    class_label = ClassLabelBasic(0)
    confidence = np.array([0.5, 0.1, 0.21, 0.19], np.float64)
    reject_threshold = 0
    learner = LearningBasic(train)
...
def test_calc_rule_weight_above_0_5():
    class_label = ClassLabelBasic(0)
    confidence = np.array([0.6, 0.1, 0.19, 0.11], np.float64)
    reject_threshold = 0
    learner = LearningBasic(train)
...
def test_calc_rule_weight_equals_threshold():
    class_label = ClassLabelBasic(0)
    confidence = np.array([0.6, 0.1, 0.19, 0.11], np.float64)
    reject_threshold = np.float64(0.2)
    learner = LearningBasic(train)

    rule_weight = learner.calc_rule_weight(class_label, confidence, reject_threshold)

    # Taken from the Java version
    assert rule_weight.get_value() == np.float64(0.0) and class_label.is_rejected()
