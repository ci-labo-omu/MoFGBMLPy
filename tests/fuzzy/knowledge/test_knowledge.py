...
def test_set_fuzzy_vars_one():
    fuzzy_vars1 = np.array(
        [FuzzyVariable(fuzzy_sets=np.array([TriangularFuzzySet(0, 0.5, 1, 0, "small")], object))], object
    )
    knowledge = Knowledge(fuzzy_vars1)
    fuzzy_sets = np.array(
        [TriangularFuzzySet(0, 0.5, 1, 2024, "small"), TriangularFuzzySet(0, 0.5, 1, 0, "small")], object
    )
    fuzzy_vars = np.array([FuzzyVariable(fuzzy_sets=fuzzy_sets)], object)
    ...
    assert (
        len(knowledge.get_fuzzy_vars()) == 1
        and knowledge.get_num_fuzzy_sets(0) == 2
        and knowledge.get_fuzzy_set(0, 0).get_id() == 2024
    )
...
def test_plot_fuzzy_variables_2_vars_1_set():
    ...
    var2 = FuzzyVariable(
        fuzzy_sets=np.array([TriangularFuzzySet(0.1, 0.5, 1.1, 1, "small")], object),
        domain=np.array([0.0, 1.1], np.float32),
        name="x1",
    )
