...
from util import get_a0_0_iris_train_test, get_a0_0_german_train_test, create_michigan_sol
...
def test_eq_true():
    assert Classifier(SingleWinnerRuleSelection()) == Classifier(SingleWinnerRuleSelection())


def test_eq_false():
    assert Classifier(SingleWinnerRuleSelection()) != Classifier(AbstractClassification())
...
def test_deep_copy():
    ...
    assert obj == obj_copy and id(obj.get_classification()) != id(obj_copy.get_classification())
