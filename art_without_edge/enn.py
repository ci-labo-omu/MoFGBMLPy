...
#以下では，Edited Nearest Neighbourを用いてデータセットを圧縮する，上のコードと同様の流れで実装する
...
output_base = Path(f"./nodes_enn/{data_name}/")
...
for train_file in train_dir.glob(f"*{data_name}-10tra.dat"):
    ...
    X_resampled, y_resampled = enn.fit_resample(X, y_encoded)
    print(len(X_resampled), len(y_resampled))
    # ラベルを元に戻す
    y_resampled_str = le.inverse_transform(y_resampled)

    # 保存ファイルパス
    output_file = output_base / f"{identifier}_enn.dat"
